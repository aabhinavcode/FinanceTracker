# finance_db.py — optimized, beginner-friendly DB helpers for your app
...
# ------------------ Connection ------------------
# Adjust if your user/pass/host/port/dbname differ
DATABASE_URL = "postgresql+psycopg2://postgres:123@localhost:5432/FinanceDatabase"

# One global engine for all operations
engine = create_engine(DATABASE_URL, pool_pre_ping=True, echo=False)

# ------------------ Schema: create tables ------------------
def create_tables():
    """
    Creates 'transactions' and 'payments' tables (if missing) with a UNIQUE row_hash
    so we can safely re-load statements without duplicating rows.
    """
...
def create_indexes_and_views():
    """
    Adds indexes for faster reads and exposes simple analytics views that
    Power BI / Streamlit can query directly.
    """
    with engine.begin() as conn:
        # Indexes (accelerate filters/group-bys)
        ...
        SELECT date_trunc('month', trans_date)::date AS month,
        ...
        # Top merchants (description hint)
        conn.execute(text("""
        ...
        FROM transactions
        GROUP BY 1
        ORDER BY total_spend DESC, txn_count DESC
        LIMIT 200;
        """))

# ------------------ Hashing helpers (dedupe) ------------------
def _norm_date(x):
    if pd.isna(x): return ""
    try:
...
def _hash_row(bank, trans_date, post_date, description, category, amount):
    """
    Stable fingerprint for a row. If any of the key fields change, the hash changes.
    """
    s = "|".join([
        _norm_text(bank),
        _norm_date(trans_date),
        _norm_date(post_date),
...
def _prepare_tx_df(df: pd.DataFrame) -> pd.DataFrame:
    """
    Accepts either Title-Case (from parser/UI) or snake_case (from DB) columns,
    computes row_hash, and renames columns to DB schema.
    """
    ...
    df2 = df.copy()
    # compute row_hash
    df2["row_hash"] = df2.apply(
        lambda r: _hash_row(
            r.get(bank_col), r.get(tcol), r.get(pcol),
    ...
    df2 = df2.rename(columns={
        ...
        pcol: "post_date",
        dcol: "description",
        ccol: "category" if ccol else "category",
        acol: "amount",
...
def _existing_hashes(table_name: str) -> set:
    """
    Pull existing row_hashes once to dedupe in Python before a bulk insert.
    Much faster than row-by-row INSERT ... ON CONFLICT.
    """
...
def upsert_transactions(df: pd.DataFrame) -> int:
    """
    Dedup in Python (by row_hash) and bulk insert the rest.
...
def read_all_transactions(limit: int | None = None) -> pd.DataFrame:
    """
    Returns all (or last N) transactions ordered by date desc.
    """
    if limit:
...
# Views for charts (DB-side aggregation = faster)
